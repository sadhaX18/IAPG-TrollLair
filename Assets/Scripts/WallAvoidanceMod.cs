...
namespace UnityMovementAI
{
    ...
    public class WallAvoidanceMod : MonoBehaviour
    {
        ...
        bool GenericCast(Vector3 direction, out GenericCastHit hit, float distance = Mathf.Infinity)
        {
            ...
            else
            {
                ...
                result = (h.collider != null && !h.collider.CompareTag("Gem") && !h.collider.CompareTag("Goal") && !h.collider.CompareTag("Torch")
                    && !h.collider.CompareTag("TrollChief") && !h.collider.CompareTag("Troll") && !h.collider.CompareTag("Thief"));
                hit = new GenericCastHit(h);
